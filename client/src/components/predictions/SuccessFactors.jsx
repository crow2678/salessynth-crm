// src/components/predictions/SuccessFactors.jsx
 ...
 * 
 * Displays the positive factors that contribute to deal success
 * 
...
const SuccessFactors = ({ 
    ...
    return (
      <div className={`bg-white rounded-lg border border-gray-200 p-6 ${className}`}>
        <div className="text-center py-6">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-1">No Success Factors</h3>
          <p className="text-sm text-gray-500">
            No positive factors have been identified yet. As more data becomes available, 
            success factors will appear here.
          </p>
        </div>
      </div>
    );
  ...
  function determineFactorType(description) {
    ...
    if (lowerDesc.includes('engage') || lowerDesc.includes('meeting') || lowerDesc.includes('call') || lowerDesc.includes('contact')) {
      return 'engagement';
    }
    if (lowerDesc.includes('budget') || lowerDesc.includes('price') || lowerDesc.includes('cost') || lowerDesc.includes('revenue') || lowerDesc.includes('roi')) {
      return 'financial';
    }
    if (lowerDesc.includes('timeline') || lowerDesc.includes('schedule') || lowerDesc.includes('date')) {
      return 'timeline';
    }
    if (lowerDesc.includes('stakeholder') || lowerDesc.includes('decision') || lowerDesc.includes('team')) {
      return 'stakeholders';
    }
    if (lowerDesc.includes('product') || lowerDesc.includes('feature') || lowerDesc.includes('solution') || lowerDesc.includes('technical')) {
      return 'product';
    }
    if (lowerDesc.includes('competitor') || lowerDesc.includes('alternative') || lowerDesc.includes('market')) {
      return 'competitive';
    }
    return 'general';
  }
            ...
            >
              ...
              {factor.impact > 1 && (
                <div className="ml-auto flex items-center">
                  {[...Array(factor.impact)].map((_, i) => (
                    <Star key={i} className={`h-3.5 w-3.5 ${textColor}`} fill="currentColor" />
                  ))}
                </div>
              )}
            </li>
          ...
          <div key={type} className="border rounded-lg p-4">
            <div className="flex items-center mb-3">
              ...
              <h4 className="font-medium text-gray-900 capitalize">
                {type === 'general' ? 'Other Factors' : `${type} Factors`}
              </h4>
  ...
  const renderImpactView = () => {
    ...
    const sortedFactors = [...normalizedFactors].sort((a, b) => b.impact - a.impact);
    
    return (
      <div className={`bg-white rounded-lg border border-gray-200 p-6 ${className}`}>
        <h3 className="text-lg font-medium text-gray-900 mb-4 flex items-center">
          <CheckCircle2 className="mr-2 h-5 w-5 text-green-500" />
          Key Success Factors by Impact
        </h3>
        ...
        <div className="space-y-4">
          {sortedFactors.map((factor, index) => {
            ...
            // Calculate impact width (20% minimum, 100% maximum)
            const impactPercentage = 20 + (Math.min(factor.impact, 5) / 5) * 80;
                  ...
                  >
                    <div className="flex items-center justify-end h-full pr-3">
                      <span className="text-xs font-semibold">
                        {['Low', 'Medium', 'High', 'Very High', 'Critical'][Math.min(factor.impact - 1, 4)]}
                      </span>
                    </div>
                  </div>
                </div>
        ...
        {data.probability && (
          <div className="mt-6 pt-4 border-t border-gray-100">
            <div className="flex items-center justify-between text-sm">
              <div className="text-gray-500">Overall Success Probability</div>
              <div className="font-semibold text-green-600">{data.probability}%</div>
            </div>
          </div>
        )}
      </div>
